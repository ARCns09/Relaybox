...
export function CreateMailboxModal({ domain, defaultLifetime, onClose, onCreate }: Props) {
  ...
  const actualLifetime = useMemo(() => lifetime === "custom" ? customValue * ({ minutes: 60, hours: 3600, days: 86400 }[customUnit]) : lifetime, [lifetime, customValue, customUnit]);
  const submit = async () => {
    setBusy(true);
  ...
  return <Modal title="Create a private mailbox" subtitle="A fresh address, protected by a token stored only in this browser." onClose={onClose} className="create-modal">
    <div className="modal-content">
      ...
      {lifetime === "custom" && <div className="custom-duration"><input type="number" min="5" max="365" value={customValue} onChange={(event) => setCustomValue(Math.max(1, Number(event.target.value)))} /><select value={customUnit} onChange={(event) => setCustomUnit(event.target.value as typeof customUnit)}><option>minutes</option><option>hours</option><option>days</option></select></div>}
